...
namespace Cosmos.HAL.Drivers.Audio
{
	...
	/// </summary>
	public sealed unsafe class waterfallES1371 : AudioDriver
	{
		...
		private waterfallES1371(ushort bufferSize)
		{
			if (bufferSize % 2 != 0)
			...
			PCIDevice pci = HAL.PCI.GetDevice(
		...
		/// </summary>
		public static waterfallES1371 Instance { get; private set; } = null;
		...
		/// <exception cref="ArgumentException">Thrown when the given buffer size is invalid.</exception>
		/// <exception cref="InvalidOperationException">Thrown when no ES1371-compatible sound card is present.</exception>
		public static waterfallES1371 Initialize(ushort bufferSize)
		{
			if (Instance != null)
			{
			...
			Instance = new waterfallES1371(bufferSize);
			return Instance;
		}
		...
		public void HandleInterrupt(ref INTs.IRQContext context)
		{
			...
			{
				WaterfallBoot.CLILogs.WriteInfo("can play");
			...
			{
				WaterfallBoot.CLILogs.WriteInfo("cannot play");
